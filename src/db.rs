...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        parsed.epics.get_mut(&epic_id).context("could not find epic in database!")?.stories.push(new_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        for story_id in &parsed.epics.get(&epic_id).context("could not find epic in database!")?.stories {
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut parsed = self.database.read_db()?;
        let epic = parsed.epics.get_mut(&epic_id).context("could not find epic in database!")?;
        let story_index = epic.stories.iter().position(|id| id == &story_id).context("story id not found in epic stories vector")?;
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        ...
        parsed.epics.get_mut(&epic_id).context("could not find epic in database!")?.status = status;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        parsed.stories.get_mut(&story_id).context("could not find story in database!")?.status = status;
